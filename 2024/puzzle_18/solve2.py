...
def solve(lines: list[str], start_steps=1024,size=7):

    failure = math.inf
    x = start_steps
    while True:
        if _solve(lines,steps=x,size=71) == failure:
            return x
        x+=1


def _solve(lines: list[str], steps=1024,size=7):
...
if __name__ == "__main__":
    with open(pathlib.Path(__file__).parent / "input.txt") as handle:
        ...
        print((solve(lines,start_steps=1024,size=71)))
